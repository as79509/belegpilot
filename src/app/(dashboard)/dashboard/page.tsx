...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([statsData, docsData, auditData]) => {
      setStats(statsData);
  ...
  async function handleReprocessStuck() {
    ...
    const reprocessRes = await fetch("/api/documents/bulk-reprocess", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ documentIds: ids }),
    });
    if (reprocessRes.ok) {
      const result = await reprocessRes.json();
      toast.success(`${result.submitted} ${de.bulk.reprocessSubmitted}`);
    }
  }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => router.push("/documents?status=needs_review")}>
            <ClipboardCheck className="h-4 w-4 mr-2" />{de.review.approve.replace("Genehmigen", "Prüfung starten")}
          </Button>
          ...
          <CardContent className="flex items-center justify-between py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-amber-800">
                {stuckCount} feststeckende Belege (hochgeladen oder fehlgeschlagen)
              </span>
            </div>
            <Button variant="outline" size="sm" onClick={handleReprocessStuck}>
              {de.bulk.reprocess}
            </Button>
          </CardContent>
        </Card>
              ...
              <TableBody>
                {recentDocs.map((doc: any) => (
                  <TableRow key={doc.id} className="cursor-pointer hover:bg-muted/50" onClick={() => router.push(`/documents/${doc.id}`)}>
                    ...
                    <TableCell className="truncate max-w-[200px]">{doc.supplierNameNormalized || doc.supplierNameRaw || de.common.noData}</TableCell>
                    <TableCell className="whitespace-nowrap">{formatCurrency(doc.grossAmount, doc.currency || "CHF")}</TableCell>
